...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    ...
    <div class="flex items-center gap-3 flex-wrap">
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open"
                    class="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#002452] text-[#002452] text-sm font-semibold hover:bg-[#002452]/5 transition-colors">
            ...
            <div x-show="open" x-transition @click.outside="open = false"
                 class="absolute left-0 top-full mt-1 z-50 bg-white border border-[#E5E7EB] rounded-xl shadow-lg overflow-hidden min-w-36">
                @foreach($anneesDisponibles as $yr)
                <a href="{{ route('rapports.bilan-annuel', ['annee' => $yr]) }}"
                   class="flex items-center gap-2 px-4 py-2.5 text-sm text-[#1F2937] hover:bg-[#F3F4F6] transition-colors">
                    <span class="material-symbols-outlined text-base text-[#002452]">bar_chart</span>
                    Bilan {{ $yr }}
                </a>
                @endforeach
            </div>
    ...
    <div class="flex items-center gap-2 flex-wrap">
        ...
        <a href="{{ route('rapports.export.csv', ['mois'=>$mois,'annee'=>$annee]) }}"
           class="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#6B7280] text-[#6B7280] text-xs font-semibold hover:bg-gray-50 transition-colors"
        ...
        <a href="{{ route('rapports.export.pdf', ['mois'=>$mois,'annee'=>$annee]) }}"
           class="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#6B7280] text-[#6B7280] text-xs font-semibold hover:bg-gray-50 transition-colors"
           title="Export PDF du mois sélectionné">
            <span class="material-symbols-outlined text-sm">picture_as_pdf</span>
            PDF mensuel
        </a>
        {{-- Séparateur --}}
        <span class="h-6 w-px bg-[#E5E7EB]"></span>
        {{-- Export comparatif période --}}
        <a href="{{ route('rapports.export.csv.comparatif', ['mois'=>$mois,'annee'=>$annee,'periode'=>$nbMois]) }}"
           class="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#006c49] text-[#006c49] text-xs font-semibold hover:bg-[#006c49]/5 transition-colors"
           title="Export CSV comparatif {{ $nbMois }} mois">
            <span class="material-symbols-outlined text-sm">table_chart</span>
            CSV {{ $nbMois }} mois
        </a>
        <a href="{{ route('rapports.export.pdf.comparatif', ['mois'=>$mois,'annee'=>$annee,'periode'=>$nbMois]) }}"
           class="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#EF4444] text-[#EF4444] text-xs font-semibold hover:bg-[#EF4444]/5 transition-colors"
           title="Export PDF comparatif {{ $nbMois }} mois avec graphiques">
            <span class="material-symbols-outlined text-sm">picture_as_pdf</span>
            PDF {{ $nbMois }} mois + graphiques
        </a>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.chartComparatif = new Chart(ctx, {
        type: 'bar',
        data: {
            labels,
            datasets: [
